...
export class AdminService {
  ...
  getAllRecord() {
    return this.fireservice.collection('reports').valueChanges({ idField: 'id' });
  }

  getAllUserRrecord(nic) {
    return this.fireservice.collection('reports', ref => ref.where('nic', '==', nic)).valueChanges({ idField: 'id' });
